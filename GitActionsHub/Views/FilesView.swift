...
// MARK: - Code Editor (UITextView with synced line numbers + syntax highlighting)
...
struct CodeEditor: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        coordinator.gutterBG = gutterBG
        coordinator.gutterContent = gutterContent
        coordinator.separator = separator
        coordinator.textView = textView

        textView.text = text
        coordinator.applyHighlighting()
        coordinator.rebuildGutter()
    ...
    class Coordinator: NSObject, UITextViewDelegate {
        ...
        func rebuildGutter() {
            ...
            let lines = (textView.text as NSString).components(separatedBy: "\n")
        ...
        func textViewDidBeginEditing(_ textView: UITextView) {
            // Plain text while typing for performance
            let plain = NSMutableAttributedString(string: textView.text, attributes: [
                .font: font,
                .foregroundColor: UIColor(Color(hex: "#E8E8E8"))
            ])
            textView.attributedText = plain
        }
        ...
        func applyHighlighting() {
            ...
            // Colors
            let kwColor   = UIColor(Color(hex: "#FF79C6"))  // keywords — pink
            let typeColor = UIColor(Color(hex: "#82AAFF"))  // types — blue
            let strColor  = UIColor(Color(hex: "#6BCB77"))  // strings — green
            let cmtColor  = UIColor(Color(hex: "#4A6785"))  // comments — gray
            let numColor  = UIColor(Color(hex: "#BD93F9"))  // numbers — purple
            let propColor = UIColor(Color(hex: "#FFD93D"))  // properties — yellow
            let defColor  = UIColor(Color(hex: "#50FA7B"))  // function defs — bright green
            ...
            let keywords = [
                ...
                "@available", "@objc", "@propertyWrapper", "@frozen",
                "nonisolated", "sendable", "borrowing", "consuming"
            ]

            // Mark keywords
            let markKeywords = ["// MARK:", "// TODO:", "// FIXME:"]

            // 1. Comments first (highest priority)
            for keyword in markKeywords {
                ...
                if let regex = try? NSRegularExpression(pattern: pattern, options: [.anchorsMatchLines]) {
                    regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                        if let r = match?.range {
                            s.addAttribute(.foregroundColor, value: cmtColor, range: r)
                            s.addAttribute(.font, value: UIFont.monospacedSystemFont(ofSize: 13, weight: .bold), range: r)
                        }
                    }
                }
            }
            // Regular comments
            if let regex = try? NSRegularExpression(pattern: "//(?! MARK:| TODO:| FIXME:).*", options: [.anchorsMatchLines]) {
                regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                    if let r = match?.range { s.addAttribute(.foregroundColor, value: cmtColor, range: r) }
                }
            }
            // Block comments /* ... */
            ...
            if let regex = try? NSRegularExpression(pattern: "\"[^\"\\\\]*(\\\\.[^\"\\\\]*)*\"") {
                regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                    if let r = match?.range { s.addAttribute(.foregroundColor, value: strColor, range: r) }
                }
            }

            // 3. Numbers (only if not already colored)
            if let regex = try? NSRegularExpression(pattern: "\\b\\d+(\\.\\d+)?\\b") {
                regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                    if let r = match?.range {
                        if s.attribute(.foregroundColor, at: r.location, effectiveRange: nil) as? UIColor == UIColor(Color(hex: "#E8E8E8")) {
                            s.addAttribute(.foregroundColor, value: numColor, range: r)
                        }
                    }
                }
            }

            // 4. @attributes like @State, @Binding etc (already in keywords but match with @)
            if let regex = try? NSRegularExpression(pattern: "@\\w+") {
                regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                    if let r = match?.range {
                        if s.attribute(.foregroundColor, at: r.location, effectiveRange: nil) as? UIColor == UIColor(Color(hex: "#E8E8E8")) {
                            s.addAttribute(.foregroundColor, value: propColor, range: r)
                        }
                    }
                }
            }
            ...
            for keyword in keywords {
                ...
                if let regex = try? NSRegularExpression(pattern: pattern) {
                    regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                        if let r = match?.range {
                            if s.attribute(.foregroundColor, at: r.location, effectiveRange: nil) as? UIColor == UIColor(Color(hex: "#E8E8E8")) {
                                s.addAttribute(.foregroundColor, value: kwColor, range: r)
                            }
                        }
                    }
                }
            ...
            // 6. Types (Capitalized words after : or ->, or after struct/class/enum/protocol)
            let typePattern = "(?<=\\b(struct|class|enum|protocol|:|->)\\s+)[A-Z][a-zA-Z0-9]+"
            if let regex = try? NSRegularExpression(pattern: typePattern) {
                regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                    if let r = match?.range {
                        if s.attribute(.foregroundColor, at: r.location, effectiveRange: nil) as? UIColor == UIColor(Color(hex: "#E8E8E8")) {
                            s.addAttribute(.foregroundColor, value: typeColor, range: r)
                        }
                    }
                }
            }

            // 7. Function definitions (word before opening parenthesis)
            let funcPattern = "\\bfunc\\s+(\\w+)"
            if let regex = try? NSRegularExpression(pattern: funcPattern) {
                regex.enumerateMatches(in: raw, range: fullRange) { match, _, _ in
                    if let r = match?.rangeAt(1) {
                        s.addAttribute(.foregroundColor, value: defColor, range: r)
                    }
                }
            }
...
// MARK: - File Editor (Fixed: synced line numbers + syntax highlighting)
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // Code editor with synced gutter + highlighting
                    CodeEditor(
                        text: $text,
