...
struct FilesView: View {
    ...
    @State private var gitOps: GitOperationsManager?
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // Fix 4: Path breadcrumb + back button
                    pathBar
                    ...
                    if fileManager.isLoading {
                        ...
                        ScrollView {
                            // Fix 5: Edit mode with move up/down arrows
        ...
        .alert("إنشاء \(isCreatingFolder ? "مجلد" : "ملف")", isPresented: $showCreateDialog) {
            ...
            Button("إنشاء") {
                if !newFileName.isEmpty { fileManager.createFile(name: newFileName, at: fileManager.currentPath.path, isDirectory: isCreatingFolder); newFileName = "" }
            }
        ...
        .onAppear {
            gitOps = GitOperationsManager(gitHubService: gitHubService)
        }
    }
...
// MARK: - File Row with move buttons
...
struct FileRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 1) {
            HStack(spacing: 0) {
                // Fix 5: Move arrows in edit mode
                if isEditMode {
                    VStack(spacing: 2) {
                    ...
                    HStack(spacing: 10) {
                        ...
                        if file.isDirectory {
                            Image(systemName: isExpanded ? "chevron.down" : "chevron.right")
                                .font(.system(size: 10, weight: .semibold)).foregroundColor(AppColors.textSecondary).frame(width: 12)
                ...
                .buttonStyle(.plain)
                .contextMenu {
                    Button { onTap(file) } label: { Label(file.isDirectory ? "فتح" : "تعديل", systemImage: "pencil") }
                    Button { onRename(file) } label: { Label("إعادة تسمية", systemImage: "pencil.circle") }
                    Divider()
                    Button(role: .destructive) { onDelete(file) } label: { Label("حذف", systemImage: "trash") }
                }
            }
...
// MARK: - File Editor with line numbers
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    HStack(spacing: 8) {
                        ...
                        Text(file.name).font(.system(size: 14, weight: .semibold, design: .monospaced)).foregroundColor(AppColors.text)
                            .environment(\.layoutDirection, .leftToRight)
                    ...
                    // Fix 5: Code editor with line numbers
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button { onSave(editedContent); hasChanges = false; dismiss() } label: {
...
struct CommitPushSheet: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    Spacer()
                    if gitOps.isLoading { ProgressView().progressViewStyle(CircularProgressViewStyle(tint: AppColors.accent)).scaleEffect(0.8) }
                }
                .padding()
                Divider().background(AppColors.border)
                        ...
                        HStack(spacing: 12) {
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                Picker("", selection: $selectedRepo) {
                                    Text("اختر...").tag("")
                                    ForEach(gitHubService.repositories) { repo in Text(repo.name).tag(repo.name) }
                                }
                        ...
                        if !gitOps.commitHistory.isEmpty {
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                ForEach(gitOps.commitHistory.prefix(3)) { commit in
                                    HStack(spacing: 10) {
                                        Text(commit.sha ?? "•••••••").font(.system(size: 11, design: .monospaced)).foregroundColor(AppColors.accent)
                ...
                Button { pushToGitHub() } label: {
                    ...
                    .background(LinearGradient(colors: canPush ? [Color(hex: "#6BCB77"), Color(hex: "#4CAF50")] : [AppColors.border, AppColors.border], startPoint: .leading, endPoint: .trailing))
        ...
        .preferredColorScheme(.dark)
        .onAppear { Task { await gitHubService.fetchRepositories() } }
    }
    
    var canPush: Bool { !commitMessage.isEmpty && !selectedRepo.isEmpty }
    
    private func pushToGitHub() {
        guard let user = gitHubService.currentUser else { return }
        // Fix 3: Collect ALL files recursively including inside folders (yellow folders)
        let files = fileManager.collectAllFiles(from: fileManager.rootFiles)
        let filesToPush = files.isEmpty ? [(path: "README.md", content: "# \(selectedRepo)\n")] : files
        Task { _ = await gitOps.commitAndPush(owner: user.login, repo: selectedRepo, branch: selectedBranch, message: commitMessage, files: filesToPush) }
    }
}
