...
struct ReposView: View {
    ...
    var statusBanner: some View {
        let isSuccess = importStatus.hasPrefix("✅")
        let isError   = importStatus.hasPrefix("❌")
        let bannerColor: Color = isSuccess ? Color(hex: "#6BCB77") : isError ? Color(hex: "#FF6B6B") : AppColors.accent
        ...
        return HStack(spacing: 10) {
            if isImporting {
                ...
                Image(systemName: isSuccess ? "checkmark.circle.fill" : isError ? "xmark.circle.fill" : "info.circle.fill")
                    .font(.system(size: 16))
            ...
            Spacer()
            if !isImporting {
                Button { withAnimation { importStatus = "" } } label: {
        ...
        .padding(12)
        .background(bannerColor.opacity(isImporting ? 0.08 : 0.1))
        .clipShape(RoundedRectangle(cornerRadius: 12))
        .overlay(RoundedRectangle(cornerRadius: 12)
    ...
    func importRepoFiles(_ repo: GitHubRepo) {
        guard let user = gitHubService.currentUser else {
            importStatus = "Error: Not logged in"
            return
        }
        isImporting = true
        ...
        Task {
            do {
                ...
                await MainActor.run { importStatus = "Reading file tree for \(repo.name)..." }
                let files = try await gitHubService.fetchRepoTree(
                    owner: user.login,
                    repo: repo.name,
                    branch: repo.defaultBranch
                )

                await MainActor.run { importStatus = "Writing \(files.count) files to device..." }

                // Step 2: Write files to local storage
                let folder = LocalFileManager.appDocumentsURL
                ...
                for f in files {
                    ...
                    written += 1
                    if written % 5 == 0 {
                        await MainActor.run { importStatus = "Writing... \(written)/\(files.count)" }
                    }
                }
                ...
                await MainActor.run {
                    ...
                    isImporting = false
                    importStatus = "✅ \(files.count) files imported from \(repo.name)"
                    withAnimation { mode = 1 }
                }
